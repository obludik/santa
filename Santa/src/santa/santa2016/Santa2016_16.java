...
public class Santa2016_16 {
    ...
    public static void main(String[] args) {    
        ...
        int checkSumLen = (2 << howManyRounds);

        int compressedPart = 0;
        ...
        for (int i = 0; i < currentPattern.length(); i++) {
            ...
            if (uncompressedLen >= LEN) {
                ...
                char[] compressed = createCheckSum(howManyRounds, currentString.substring(compressedPart, 
                        currentString.length()).toCharArray());
                currentString = new StringBuilder(currentString.toString().substring(0, compressedPart) + String.valueOf(compressed));
                break;
            } else {
                currentString.append(temp.toString());
                if (currentString.length() - compressedPart >= checkSumLen) {
                    char[] compressed = createCheckSum(howManyRounds, currentString.substring(compressedPart, compressedPart + checkSumLen).toCharArray());
                    currentString = new StringBuilder(currentString.toString().substring(0, compressedPart) + String.valueOf(compressed) 
                            + currentString.toString().substring(compressedPart  + checkSumLen,  currentString.length()));
                    compressedPart += compressed.length;
                } 
            }
        }
